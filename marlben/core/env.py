...
import marlben
from marlben import entity, core
from marlben.core.map_generation import base as terrain
from marlben.infrastructure import DataType
   ...
   """
    ...
    def __init__(self, config=None):
      ...
      '''
        ...
        if not config.MAP_GENERATOR:
            config.MAP_GENERATOR = marlben.core.map_generation.base.MapGenerator

        self.realm = core.Realm(config)
        self.registry = marlben.OverlayRegistry(config, self)
    ...
    def observation_space(self, agent: int):
      ...
      Returns:
        ...
        observation = {}
        for entity in sorted(marlben.Serialized.values()):
    ...
    def action_space(self, agent):
      ...
      Returns:
        ...
        actions = {}
        for atn in sorted(marlben.Action.edges):
            actions[atn] = {}
      ...
      '''
        ...
        for entID, ent in self.realm.players():
            ...
            if ent.agent.scripted:
                atns = ent.agent(ob)
                for key in atns:
                    if marlben.action.Target in atns[key]:
                        atns[key][marlben.action.Target] = self.realm.entity(
                            atns[key][marlben.action.Target])
    ...
    def render(self, mode='human') -> None:
      ...
      '''
        ...
        if not self.client:
            from marlben.websocket import Application
            ...
            for c in range(C):
                ...
                if n == 0:
                    self.realm.dataframe.remove(
                        marlben.Serialized.Entity, entID, ent.pos)
