...
def _walk_and_paint(important_cells, mask, reached_cells):
    ...
    while new_reached_cells.sum() > 0:
        ...
        new_passed_cells = np.logical_and(
            new_passed_cells, np.logical_not(reached_cells))
...
def check_connectivity(map, start_points, connectivity_type):
    important_cells = np.logical_or(
        map[:, :, 0] == Terrain.FOREST, map[:, :, 0] == Terrain.WATER)
    mask = np.logical_and(map[:, :, 0] != Terrain.WATER, np.logical_and(
        map[:, :, 0] != Terrain.STONE, map[:, :, 0] != Terrain.LAVA))
