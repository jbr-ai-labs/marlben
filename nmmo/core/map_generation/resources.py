...
def generate_resources(tile_map, resource_type, resource_amount, visibility_colors, accessibility_colors,
    ...
    visibility_colors_counts = np.zeros(
        visibility_colors) if visibility_colors > 0 else None
    accessibility_colors_counts = np.zeros(
        accessibility_colors) if accessibility_colors > 0 else None
    ...
    for i in range(resource_amount):
        ...
        if visibility_colors > 0:
            tile_map[x, y, 1] = _get_random_color(
                visibility_colors_counts, visibility_distribution, random)
        if accessibility_colors > 0:
            tile_map[x, y, 2] = _get_random_color(
                accessibility_colors_counts, accessibility_distribution, random)
